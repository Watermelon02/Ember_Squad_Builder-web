...
interface TacticTabContentProps {
  team: Team;
  ...
  return (
    <AnimatePresence mode="wait">
      <TabsContent value="tacticCards" className="flex-1 overflow-y-auto p-4 space-y-0">
        <motion.div
          style={{ display: "grid", gridTemplateColumns: mobileOrTablet ? "repeat(2, 1fr)" : "repeat(4, 1fr)", gap: "1rem" }}
        ...
        >
          {team.tacticCards?.map((tacticCards, index) => (
            ...
            >
              ...
              {/* 如果是比赛模式，战术卡全显示背面 */}
              {tournamentMode &&
                <img src={`${tabsrc}/tactic.webp`} alt={tacticCards.name} className="shadow-lg shadow-gray-500 rounded-lg" style={{ width: '100%', height: '100%', objectFit: 'contain' }} draggable={false} />
              }
